...
```bash
docker run -e APP_NAME=fiesta_today_api \
           -e DATABASE_CLIENT=mongo \
           -e DATABASE_HOST=strapi-mongo \
           -e DATABASE_PORT=27017 \
           -e DATABASE_NAME=strapi \
           -v `pwd`/fiesta_today_api:/usr/src/api/fiesta_today_api \
...
- `APP_NAME` to override the `fiesta_today_api` generated folder name (you should also update the volumes paths).
